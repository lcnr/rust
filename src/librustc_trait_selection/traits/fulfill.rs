...
use rustc_middle::ty::{self, Const, ToPredicate, Ty, TypeFoldable};
    ...
    fn process_obligation(
        ...
        match obligation.predicate.kint(infcx.tcx) {
            &ty::PredicateKint::ForAll(binder) => {
                // For example `for<'a> T: Trait<'a>`, we convert 'a into a placeholder and then try to
                // fulfill the inner predicate in a later loop.
            ...
            &ty::PredicateKint::Projection(data) => {
                ...
                match project::project_and_unify_type(self.selcx, &project_obligation) {
                    Ok(None) => {
                        let tcx = self.selcx.tcx();
                        pending_obligation.stalled_on = trait_ref_infer_vars(
                            self.selcx,
                            data.projection_ty.trait_ref(self.selcx.tcx()),
                        );
                        ProcessResult::Unchanged
                    }

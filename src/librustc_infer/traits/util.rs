...
pub fn anonymize_predicate<'tcx>(
    ...
    match pred.kind() {
        ty::PredicateKind::ForAll(binder) => {
            ty::PredicateKind::ForAll(tcx.anonymize_late_bound_regions(binder)).to_predicate(tcx)
        }
        // All other predicates do not contain a binder, so we can return them as is.
        ...
        | ty::PredicateKind::ConstEquate(..) => pred,
    }
}
...
impl Elaborator<'tcx> {
    ...
    fn elaborate(&mut self, obligation: &PredicateObligation<'tcx>) {
        ...
        // We currently do not care about higher ranked bounds here.
        // TODO: I am not sure how good of an idea this is.
        match obligation.propagate_binder().kind() {
            ty::PredicateKind::ForAll(..) => bug!("unexpected obligation: {:?}", obligation),
...
impl<'tcx, I: Iterator<Item = PredicateObligation<'tcx>>> Iterator for FilterToTraits<I> {
    ...
    fn next(&mut self) -> Option<ty::PolyTraitRef<'tcx>> {
        while let Some(obligation) = self.base_iterator.next() {
            if let Some(poly_trait_ref) = obligation.predicate.to_opt_poly_trait_ref() {

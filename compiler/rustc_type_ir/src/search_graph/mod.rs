...
impl<X: Cx> EvaluationResult<X> {
    fn finalize(
        ...
        EvaluationResult {
            ...
            // The `node_id` should stay the same between all iterations, we just
            // use the final one.
...
impl<D: Delegate<Cx = X>, X: Cx> SearchGraph<D> {
    ...
    fn evaluate_goal_in_task(
        ...
        let input = self.stack.last().unwrap().input;
        ...
        let Some(usage_kind) = stack_entry.has_been_used else {
            if cfg!(debug_assertions) {
                #[allow(rustc::potential_query_instability)]
                for (_, entries) in &self.provisional_cache {
                    for entry in entries {
                        assert!(entry.heads.highest_cycle_head() < self.stack.next_index());
                    }
                }
            }
        ...
        loop {
            ...
            if i >= D::FIXPOINT_STEP_LIMIT {
                ...
                let result = D::on_fixpoint_overflow(cx, stack_entry.input);
                Self::rebase_provisional_cache_entries(
                    &self.stack,
                    &mut self.provisional_cache,
                    &stack_entry,
            ...
            match self.reevaluate_goal_step(cx, stack_entry, result, inspect) {
    ...
    fn reevaluate_goal_step(
        ...
        let next_depth = self.stack.next_index();
        ...
        let mut did_fully_reeval = HashSet::default();
        'cycles: for cycle in self.tree.rerun_get_and_reset_cycles(node_id, provisional_result) {
            let cycle_id = self.tree.get_cycle(cycle);

            match *self.tree.node_kind_raw(cycle_id) {
                ...
                tree::NodeKind::ProvisionalCacheHit { entry_node_id } => {
                    // The cache entry does not depend on the goal we're rerunning right now.
                    if entry_node_id < node_id {
                        continue 'cycles;
                    }

                    if !self.tree.node_depends_on_head(entry_node_id, current_depth) {
                        continue 'cycles;
                    }
                }
            }
            ...
            for depth in (next_depth.as_usize()..).map(StackDepth::from_usize) {
                ...
                if did_fully_reeval.contains(&entry.node_id) {
                    continue 'cycles;
                } else {
                    ...
                    self.stack.push(StackEntry {
                        ...
                        step_kind_from_parent: entry.info.step_kind_from_parent,
                        ...
                        rerun_info: Some((entry.node_id, entry.is_final_iteration)),
                    });
                }
            }
        ...
        self.pop_stack_until(cx, &mut needs_reeval, &mut did_fully_reeval, next_depth, inspect);
        match needs_reeval.as_slice() {
            &[d] => {
                debug_assert_eq!(d, current_depth);
                let result = D::compute_goal(self, cx, prev_stack_entry.input, inspect);
                let reeval_entry = self.stack.pop();
                (reeval_entry, result)
            }
            [] => {
                let unchanged_entry = self.stack.pop();
                (unchanged_entry, provisional_result)
            }
            _ => unreachable!("unexpected needs_reeval: {needs_reeval:?}"),
        }
    }
    ...
    fn pop_stack_until(
        &mut self,
        cx: X,
        needs_reeval: &mut Vec<StackDepth>,
        did_fully_reeval: &mut HashSet<tree::NodeId>,
        depth: StackDepth,
        inspect: &mut D::ProofTreeBuilder,
    ) {

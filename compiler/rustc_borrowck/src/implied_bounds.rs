...
use rustc_middle::ty::{
    self, CanonicalVarValues, GenericArg, GenericArgs, List, RegionExt, Ty, TyCtxt,
    TypeVisitableExt, TypingEnv, fold_regions,
};
...
> {
    ...
    // We choose `'erased` for the `c_variadic_region` to later replace it with a unique placeholder.
    let inputs_and_output = defining_ty.inputs_and_output(tcx, || tcx.lifetimes.re_erased);
    let late_bound_vars = inputs_and_output.bound_vars();
    ...
    let var_values = implied_bounds_query_var_values(
        tcx,
        body_def_id,
        late_bound_vars,
        &inputs_and_output,
...
///
/// This has to be kept in sync between the caller and the query itself. It's
/// unfortunate that we manually compute this instead of being a query argument.
/// However, this is different from a normal canonical query as we know the current
/// context even inside of the query. This setup is probably not ideal, but it does
/// work :>
///
/// The `var_values` should be:
/// - early bound parameters of the body
/// - late bound parameters of the body itself
/// - all the external regions in the `DefiningTy` of the body
#[instrument(level = "debug", skip(tcx, is_external_region), ret)]
pub(crate) fn implied_bounds_query_var_values<'tcx>(
    tcx: TyCtxt<'tcx>,
    body_def_id: LocalDefId,
    late_bound_vars: &'tcx List<ty::BoundVariableKind<'tcx>>,
    ...
    let mut values: SmallVec<[GenericArg<'tcx>; 8]> =
        GenericArgs::identity_for_item(tcx, body_def_id).into_iter().collect();

    values.extend(late_bound_vars.iter().enumerate().filter_map(|(idx, bound_var)| {
        // FIXME(non_lifetime_binders): We don't support non-lifetime binders on closures.
        let ty::BoundVariableKind::Region(kind) = bound_var else {
            return None;
        };
        let kind = ty::LateParamRegionKind::from_bound(ty::BoundVar::from_usize(idx), kind);
        Some(ty::Region::new_late_param(tcx, body_def_id.to_def_id(), kind).into())
    }));

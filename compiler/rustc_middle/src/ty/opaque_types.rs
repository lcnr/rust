...
impl<'tcx> TypeFolder<TyCtxt<'tcx>> for ReverseMapper<'tcx> {
    ...
    fn fold_region(&mut self, r: ty::Region<'tcx>) -> ty::Region<'tcx> {
        match *r {
            // Ignore bound regions as these are local to the hidden type.
            //
            // E.g. We ignore `'r` in a type like `for<'r> fn(&'r u32)`.
            ty::ReBound(..) => return r,
            // We also don't have to remap any other regions not local
            // to the function.
            ty::ReStatic | ty::ReError(_) => return r,

            // If regions have been erased, just keep them this way.

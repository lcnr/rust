...
rustc_queries! {
    ...
    /// Computes implied bounds and the normalized signature outside the NLL inference context.
    query mir_borrowck_implied_outlives_bounds(
        ...
        NoSolution,
    > {
        desc { "computing implied outlives bounds for `{}`", tcx.def_path_str(mir_def) }
    }

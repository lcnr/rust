...
use rustc_search_graph::{self as sg, Delegate, UsageKind};
...
impl<'tcx> Delegate<SearchGraphDelegate> for TyCtxt<'tcx> {
    ...
    fn is_initial_provisional_result(self, usage_kind: UsageKind, result: Self::Result) -> bool {
        match usage_kind {
            UsageKind::Coinductive => {
                is_response_no_constraints(result, |certainty| certainty == Certainty::Yes)
            }
            UsageKind::Inductive => is_response_no_constraints(result, |certainty| {
    ...
    fn opt_initial_provisional_result(
        ...
        match usage_kind {
            UsageKind::Coinductive => Some(response_no_constraints(self, input, Certainty::Yes)),
            UsageKind::Inductive => {
                Some(response_no_constraints(self, input, Certainty::overflow(false)))
            }
            UsageKind::Both => None,
        }
    }
    ...
    fn reached_fixpoint(
        ...
        if is_response_no_constraints(result, |c| matches!(c, Certainty::Maybe(_))) {
            ...
            r == result
        } else if usage_kind == UsageKind::Coinductive {

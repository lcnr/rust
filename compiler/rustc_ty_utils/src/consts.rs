...
fn recurse_build<'tcx>(
    ...
    Ok(match &node.kind {
        ...
        &ExprKind::Literal { lit, neg } => {
            match tcx.at(node.span).lit_to_const(LitToConstInput {
                lit: &lit.node,
                ty: node.ty,
                neg,
            }) {
                ...
                Err(LitToConstError::Reported) => {
                    return Err(tcx
                        .sess
                        .delay_span_bug(root_span, "Failed to convert literal to constant"));
                }
        ...
        &ExprKind::NamedConst { def_id, substs, user_ty: _ } => {
            ...
            tcx.mk_const(ty::ConstS { kind: ty::ConstKind::Unevaluated(uneval), ty: node.ty })
        }
        ExprKind::ConstParam { param, .. } => {
            tcx.mk_const(ty::ConstS { kind: ty::ConstKind::Param(*param), ty: node.ty })
        }
        ...
        ExprKind::Call { fun, args, .. } => {
            ...
            tcx.mk_const(ty::ConstS {
                kind: ConstKind::Expr(Expr::FunctionCall(fun, new_args)),
                ty: node.ty,
            })
        }
        &ExprKind::Binary { op, lhs, rhs } if check_binop(op) => {
            ...
            tcx.mk_const(ty::ConstS {
                kind: ConstKind::Expr(Expr::Binop(op, lhs, rhs)),
                ty: node.ty,
            })
        }
        &ExprKind::Unary { op, arg } if check_unop(op) => {
            let arg = recurse_build(tcx, body, arg, root_span)?;
            tcx.mk_const(ty::ConstS { kind: ConstKind::Expr(Expr::UnOp(op, arg)), ty: node.ty })
        }
        ...
        &ExprKind::Use { source } => {
            ...
            tcx.mk_const(ty::ConstS {
                kind: ConstKind::Expr(Expr::Cast(CastKind::Use, arg, node.ty)),
                ty: node.ty,
            })
        }
        &ExprKind::Cast { source } => {
            let arg = recurse_build(tcx, body, source, root_span)?;
            tcx.mk_const(ty::ConstS {
                kind: ConstKind::Expr(Expr::Cast(CastKind::As, arg, node.ty)),
                ty: node.ty,
            })
        }
        ExprKind::Borrow { arg, .. } => {
